from src.functions import transaction
...
def test_transaction():
    assert transaction(all_list[0]) == 'Bank -> Счет **5907'
    assert transaction(all_list[1]) == 'Visa Classic 2842 87** **** 9012 -> Счет **3655'
    assert transaction(all_list[2]) == 'Maestro 7810 84** **** 5568 -> Счет **2869'
    assert transaction(all_list[3]) == 'Счет **9794 -> Счет **8125'
    assert transaction(all_list[4]) == 'Bank -> Счет **8381'