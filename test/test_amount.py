from src.functions import amount
...
def test_amount():
    assert amount(all_list[0]) == '41096.24 USD'
    assert amount(all_list[1]) == '48150.39 USD'
    assert amount(all_list[2]) == '30153.72 руб.'
    assert amount(all_list[3]) == '62814.53 руб.'
    assert amount(all_list[4]) == '21344.35 руб.'